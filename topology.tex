...
\begin{example}
$X=[0,2]$和$Y=[0,1] \cup [2,3]$不同胚，在$Y$上可以$[0,1]$是一个clopen($[0,1] = [0,1] \cap Y$，$[0,1] = (-1,-\frac{3}{2}) \cap Y$)，如果$X$和$Y$同胚，则$f^{-1}([0,1])$也应该在$X$上是个clopen，但是$X$连通，所以这样的$f$不存在.
\end{example}

这里可以引出$\mathbb{R}$上哪样的子空间是连通的一般性的性质.
