...
\begin{proof}
若前提条件满足，假设$X$不是完全有界的，则存在一个$\varepsilon$，使得无法用有限多个$B_\varepsilon(x_i)$开球盖住. 取$x_1 \in X$，因为$B_\varepsilon(x_1)$无法用盖住$X$，所以可以再取一点$x_2 \in X \ B_\varepsilon(x_1)$. by induction，我们可以一直这样取下去，因为$Y \ \bigcup\limits_{i=1}^{n} B_\varepsilon(y_i)$不为空.这样我们找到一个序列$\{x_n\}$，使得$d(x_m,x_n) \geq \varepsilon,m \neq n$.由收敛原理这样的$\{x_n\}$并不收敛，与前提矛盾.
\end{proof}
...
\begin{proposition}
所有完备有界(complete totally bounded)的度量空间是紧致的.
\end{proposition}
