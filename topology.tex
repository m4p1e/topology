...
\begin{proof}
这个命题值得证明一下,从前面那个lemma我们知道，如果$A$和真子集基数相等，那它肯定不是有限集。反过来$A$如果不是有限集，假设$A$有一系列元素$a_1,a_2,\ldots$(两两不相等),然后我取它的真子集$A \smallsetminus {a_1}$，然后呢建立一连串从$a_i \in A$到$a_{i+1} \in B$的一一映射，并把除此之外的每个$b \in A$映到$b$(为什么呢？)。
\end{proof}
