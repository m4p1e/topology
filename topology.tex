...
\begin{definition}
给定一个directed set $A$和拓扑空间$X$，映射$v \colon A \mapsto X$就被称为$X$上的一个net.  常常用$x.= x_\alpha(\alpha \in A)$表示.
\end{definition}
...
\begin{proposition}
\rm 拓扑空间$X$是Hausdorff当且仅当其上的所有net最多只有一个聚点(limit point)
\end{proposition}
...
\begin{proof}
($\Leftarrow$) 前提条件成立下，我们假设$f$不是连续函数. 则存在$f(x_0)$的一个开集$V$，它对应的preimage $K$不是$x_0$的开集. 也就是说$x_0 \in K \setminus K^\circ$. 我们现在让$x_0$的所有开集是一个directed set $D$. 其relation $\geq$ 为 $\subseteq$(越大的让它越靠近$x_0$). 对于任意$A \in D$，$A$ 都是不entirely lies on $K$. 现在我们可以构造一个net $x_A \in A \setminus K$.很明显$x_0$是net的一个limit point. 但是现在$x_A$都是不在$K$里面的，所以$f(x_A) \notin V$. 也就是说$f(x_A)$不会eventually lies on $V$. 与前提条件矛盾.
\end{proof}
\end{document}
